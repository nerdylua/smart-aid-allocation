  ...
  return (
    ...
    >
      <svg
        ...
        className={cn('text-yellow-400', size === 'small' ? 'w-4 h-4' : 'w-8 h-8')}
      >
        <path
    ...
    <div className="relative grid md:grid-cols-2 gap-8 items-center">
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col items-center mt-2">
            ...
            <div className="text-xl text-muted-foreground mt-2">Github Stars</div>
          </div>
        </motion.a>
      </motion.div>
      ...
      >
        <div className="space-y-4">
          <div>
            <div className="text-3xl font-bold">{contributors.length}+ Contributors</div>
            <div className="text-lg text-muted-foreground mt-2">Join our growing community</div>
          </div>
          <a
    ...
    <section className="container mx-auto relative py-20">
      <div className="text-center mb-16">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-4xl font-bold tracking-tight sm:text-6xl mb-4">{title}</h2>
          <p className="text-xl text-muted-foreground max-w-[800px] mx-auto">{description}</p>
          <div className="mt-6">
      ...
      <div className="max-w-4xl mx-auto">
        <OpenSourceCard repository={repository} stars={stars} contributors={contributors} />
      </div>
    </section>
  )
}
...
function OpenSourceData({
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: () => getGithubStats(repository, githubToken),
...
function OpenSource(props: OpenSourceProps) {
  const { defaultStats, ...rest } = props

  return (
    <Suspense
      fallback={
        <OpenSourceContent
          stars={defaultStats?.stars || 0}
